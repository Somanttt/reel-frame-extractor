...
def extract_frames():
    """
    Receives Zernio webhook JSON, extracts reel URL, downloads reel, extracts frames.
    Returns JSON with URLs to access frames.
    """
    try:
        # Parse incoming JSON
        ...
        if result.returncode != 0:
            ...
            return jsonify({"error": f"Failed to download reel: {result.stderr}"}), 400
        
        if not reel_path.exists():
            return jsonify({"error": "Reel downloaded but file not found"}), 400
        ...
        if result.returncode != 0:
            ...
            return jsonify({"error": f"Failed to extract frames: {result.stderr}"}), 400
        
        # Step 3: Collect extracted frames
        frames = sorted(session_frames_dir.glob("frame_*.jpg"))
        
        if not frames:
            return jsonify({"error": "No frames extracted from reel"}), 400
        ...
        for idx, frame_file in enumerate(frames, 1):
            frame_url = f"{base_url}/frames/{session_id}/frame_{idx:03d}.jpg"
            frame_data.append({
                "index": idx,
                "url": frame_url,
